...
class Model:
    def __init__(self, basic_lstm_cell):
        ...
        if basic_lstm_cell == tf.contrib.rnn.BasicLSTMCell:
            self.outputs, _ = tf.nn.static_rnn(cell=lstm_cell,
                                               ...
                                               dtype=tf.float32)
            self.hidden = self.outputs[-1]
        elif basic_lstm_cell == rnn.BasicLSTMCell:
            state_tuple = lstm_cell.zero_state(batch_size)
            for input in inputs_unstacked:
                state_tuple = lstm_cell(input, state_tuple)
                self.hidden = state_tuple[1]
        else:
            NotImplementedError

        self.loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.hidden,
    ...
    def predict(self, inputs):
        prediction = self.session.run(tf.argmax(self.hidden,1), feed_dict={self.inputs: inputs})
        return prediction[0]
